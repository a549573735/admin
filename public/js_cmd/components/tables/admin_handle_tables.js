        ...
        template:'<table class="table  table-hover table-borderless">\
                      ...
                      <tbody class="v-tabs-check">\
                        <tr v-for="item in datalist.content" :class="\'tr-\'+$index" v-cloak>\
                            ...
                            <td v-if="datalist.btns" class="text-center"><div class="bei-zhu"><a  :data-admin="item.admin"   :data-id="item.id" :data-parentId="item.parentId" :data-phone="item.phone" data-type="modify" :data-contact="item.contact" :data-mail="item.mail" :data-username="item.username" :data-name="item.name" :data-address="item.address" class="btn  btn-primary " data-toggle="modal" data-target="#modal-addParty" @click="handleData($event)" > 修改</a></div><div class="bei-zhu"><a  @click="resetPassword($event)"  :data-id="item.id" :data-admin="item.admin" class="btn  btn-primary "> 重置</a></div><div class="bei-zhu"><a  @click="deleteEle($event)"  data-toggle="modal"  data-target="#modal-DeletePrompt" :data-id="item.id" :data-admin="item.admin" class="btn  btn-primary "> 删除</a></div></td>\
        ...
        methods:{
             ...
             deleteEle:function (event){
                   var form={
                    ...
                    $('#delete-ground').attr({'data-tr':tr.attr('class'),'data-id':form.id,'data-api':'/admin/market/delete'})
